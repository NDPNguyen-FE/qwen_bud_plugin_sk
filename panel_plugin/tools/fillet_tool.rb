...
module PanelPlugin
  module Tools
    class FilletTool
      ...
      def update_status
        Sketchup.status_text = "Cắt Cạnh Góc (Fillet/Chamfer). Click vào CẠNH GÓC dọc của ván để chọn, sau đó nhập kích thước."
      end
      ...
      def onLButtonDown(flags, x, y, view)
        if @hover_edge && @hover_instance
          ...
          if r > 0 && s >= 1
            ...
            Sketchup::UI.messagebox("Kích thước phải > 0 và Segments phải hợp lệ")
          end
        end
      end
      ...
      def do_fillet(instance, edge, segments, cut_type, size_mm)
        ...
        unless profile_face
          Sketchup::UI.messagebox("Không tìm thấy mặt phẳng đầu hồi (Profile Face) thích hợp ở 2 đầu cạnh này để bo.")
          return
        end
        ...
        if edges_on_profile.length < 2 
          Sketchup::UI.messagebox("Cạnh này không hợp lệ (không tạo thành góc đỉnh trên mặt).")
          return
        end
        ...
        if angle <= 0.01 || angle > 179.degrees
           Sketchup::UI.messagebox("Góc quá thẳng, không thể tác động.")
           return
        end
        ...
        if d > vec1.length || d > vec2.length
          Sketchup::UI.messagebox("Kích thước #{size_mm}mm quá lớn so với độ dài của cạnh mặt hồi!")
          return
        end
