    ...
    class SmartDividerTool
      ...
      def _process(face)
        ...
        unless %w[H V].include?(direction)
          Sketchup::UI.messagebox("Hướng chia phải là 'H' (ngang) hoặc 'V' (dọc).")
          return
        end
      ...
      def _validate(direction, n, thick_mm, inset_mm, inner_w_su, inner_d_su, inner_h_su)
        ...
        if n <= 0
          Sketchup::UI.messagebox('Số lượng phải lớn hơn 0.')
          return false
        end

        if n > 50
          Sketchup::UI.messagebox('Số lượng không được vượt quá 50.')
          return false
        end

        if thick_mm < 3.0
          Sketchup::UI.messagebox('Độ dày phải >= 3 mm.')
          return false
        end

        if inset_mm < 0
          Sketchup::UI.messagebox('Hụt vào không được âm.')
          return false
        end
        ...
        if direction == 'H'
          ...
          if net_w < 50.0
            Sketchup::UI.messagebox("Chiều rộng hữu dụng quá nhỏ: #{net_w.round(1)} mm.")
            return false
          end
          if inner_d_mm < 50.0
            Sketchup::UI.messagebox("Chiều sâu khoang quá nhỏ: #{inner_d_mm.round(1)} mm.")
            return false
          end
          ...
          if clear_gap_mm < 30.0
            Sketchup::UI.messagebox("Không đủ khoảng hở giữa các đợt (cần >= 30mm).")
            return false
          end
        else
          ...
          if net_d < 50.0
            Sketchup::UI.messagebox("Chiều sâu hữu dụng quá nhỏ: #{net_d.round(1)} mm.")
            return false
          end
          if inner_w_mm < 50.0
            Sketchup::UI.messagebox("Chiều rộng khoang quá nhỏ: #{inner_w_mm.round(1)} mm.")
            return false
          end
          ...
          clear_gap_mm = spacing_mm - thick_mm
          if clear_gap_mm < 30.0
            Sketchup::UI.messagebox("Không đủ khoảng hở giữa các vách (cần >= 30mm).")
            return false
          end
        end
